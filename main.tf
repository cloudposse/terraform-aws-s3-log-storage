...
module "aws_s3_bucket" {
  ...
  logging = var.access_log_bucket_name != null && var.access_log_bucket_name != "" ? {
    ...
    prefix      = var.access_log_bucket_prefix_override == null ? "${join("", data.aws_caller_identity.current[*].account_id)}/${module.context.id}/" : (var.access_log_bucket_prefix_override != "" ? "${join("", data.aws_caller_identity.current[*].account_id)}/${module.context.id}/" : "")
  } : {}
